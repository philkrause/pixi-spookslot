...
class Reel extends PIXI.Sprite{
  ...
  protected setPosition() {
    this.x = this.reelIndex * this.width
    this.y = 0;
  }

  protected setTilePositionAt(cellNumber: number) {
    this.x = 0;
    this.y = -cellNumber * this.cellHeight
  }
  ...
  //   this.spinningOutcome = useOutcome >= 0 ? useOutcome : 0
  ...
  spinAnimation(useOutcome: number, spinningSpeedFactor: number, cb: Function) {
    
    console.log(`SpeedFactor: ${this.appTicker}`)
    ...
    const animation = () => {
      ...
      if(this.spinningCyclesMeter < 0) {
        this.y += this.spinningCyclesMeter;
        this.spinningCyclesMeter = 0;
  ...
  //     const outcome = this.finalcells[0].getSpinningOutcome();
  //     //won = this.finalcells.find((i) => i.getSpinningOutcome() !== outcome) === undefined;
  //   }
